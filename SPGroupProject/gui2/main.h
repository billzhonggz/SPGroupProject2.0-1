...
void GUI_Entrance(struct items* head);//The Entrance of user interface. Deal with the return values of GUI functions and call functions. 
int Login();//Choosing Customer or Manager GUI.
int GUI_CustomerMain(struct items* head);//Customer interface.
int GUI_CustomerNumber(struct items* head, int itemID);//To confirm item number buy by customer.
...
int GUI_ManagerPwd();//Manager submenu. Change password.
